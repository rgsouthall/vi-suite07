...
if "bpy" in locals():
    ...
    try:
        ...
        if sys.platform == 'darwin':
            ...
            ng_cmd = '"{0}" -m pip install --upgrade --force --target "{1}" netgen-mesher==6.2.2503'.format(sys.executable, os.path.join(addonpath, 'Python', sys.platform))
            ...
            Popen(shlex.split(ngocc_cmd)).wait()
            ng_cmd = '"{0}" -m pip install --target "{1}" netgen-mesher==6.2.2503'.format(sys.executable, os.path.join(addonpath, 'Python', sys.platform))
            Popen(shlex.split(ng_cmd)).wait()

        elif sys.platform == 'linux':
            if not sys_install:
                ng_cmd = '"{0}" -m pip install --upgrade netgen-mesher==6.2.2503'.format(sys.executable)
                Popen(shlex.split(ng_cmd)).wait()
            else:
